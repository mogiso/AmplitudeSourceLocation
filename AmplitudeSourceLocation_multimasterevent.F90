program AmplitudeSourceLocation_multimasterevent
  ...
  !!Copyright: (c) Masashi Ogiso 2026
  ...
  !!use multiple master event, no traveltime table
  !!use particlefilter for searching best location
  ...
  ot_tmp = ot_begin
  count_loop: do
    if(ot_tmp + rms_tw .gt. ot_end) exit
  ...
  do k = 1, nsubevent
    ...
    do j = 1, nsta
      ...
      do i = 1, int(rms_tw / sampling(j) + 0.5_fp)
        time_index = int((ot_tmp - begin(j)) + i
     ...
     !!do particlefilter
     particlefilter_search_count = 0
     particlefilter_search: do
       if(particlefilter_search_count .gt. particlefilter_search_count_max) exit particlefilter_search

       if(particlefilter_search_count .eq. 0) then
         !!initialize particles
         do i = 1, nparticle
           call gen_random_number(randomnumber_status, rnd)
           particle_loc(i)%lon = particle_lon_w + (particle_lon_e - particle_lon_w) * rnd
           particle_loc(i)%xeast = (particle_loc%lon - mean_lon) * deg2rad &
           &                     * (r_earth - mean_depth) * sin(pi * 0.5_fp - mean_lat * deg2rad)
           call gen_random_number(randomnumber_status, rnd)
           particle_loc(i)%lat = particle_lat_s + (particle_lat_n - particle_lat_s) * rnd
           particle_loc(i)%ynorth = (particle_loc(i)%lat - mean_lat) * deg2rad * (r_earth - mean_depth)
           call gen_random_number(randomnumber_status, rnd)
           particle_loc(i)%depth = particle_depth_min + (particle_depth_max - particle_depth_min) * rnd
         enddo
       else
       ...
       do k = 1, nparticle
         do j = 1, nev_master
           do i = 1, nsta
             ...
             normal_vector(1 : 3) = [sin(ray_azinc(2, i, j)) * cos(ray_azinc(1, i, j)), &  !!latitude (ynorth)
             &                       sin(ray_azinc(2, i, j)) * sin(ray_azinc(1, i, j)), &  !!longitude (xeast)
             &                       cos(ray_azinc(2, i, j))]                              !!depth (down+)
             matrix_const = pi * freq * evloc_master(j)%qinv / evloc_master(j)%vel + 1.0_fp / hypodist(i, j)
 

       particlefilter_search_count = particlefilter_search_count + 1
     enddo particlefilter_search
  ...
  stop
end program AmplitudeSourceLocation_multimasterevent
