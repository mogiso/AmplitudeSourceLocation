...
contains
  ...
  &                          pulsewidth)
    ...
    do i = 1, nraypath
      ...
      call latgtoc(raynode(i)%lat, raynode(i)%theta); raynode(i)%theta = pi * 0.5_fp - raynode(i)%theta
    enddo
    ...
    raypath_divide: do j = 1, ndiv_raypath
      ...
      if(nraypath .ne. 2) then
        ...
        raybend_loop: do
          ...
          !!calculate traveltime of the bended ray
      ...
      do i = 2, nraypath - 1, 2
        ...
        call latctog(pi * 0.5_fp - raynode(i)%theta, raynode(i)%lat)
      enddo

      !!calculate traveltime of the ray which nodes are doubled
  ...
  subroutine move_node(node1, node2, node3, velocity, lon_w, lat_s, dep_min, dlon, dlat, ddep, enhance_factor)
    ...
    if(dist_move .eq. 0.0_fp) then
      node2%r     = node_tmp%r
      node2%theta = node_tmp%theta
      node2%phi   = node_tmp%phi
    else
  ...
  &                                  pulsewidth_element)
    ...
    if(present(pulsewidth_element)) then
      ...
      lon_grid(1) = lon_w_qinv   + dlon_qinv * real(lon_index - 1, kind = fp); lon_grid(2) = lon_grid(1) + dlon_qinv
      lat_grid(1) = lat_s_qinv   + dlat_qinv * real(lat_index - 1, kind = fp); lat_grid(2) = lat_grid(1) + dlat_qinv
      z_grid(1)   = dep_min_qinv + ddep_qinv * real(z_index - 1,   kind = fp); z_grid(2)   = z_grid(1)   + ddep_qinv
